---
title: "R Notebook 6.25"
output:
  html_document:
    df_print: paged
---

This is an [R Markdown](http://rmarkdown.rstudio.com) Notebook containing the logistic mixed-model analyis for **WORKING TITLE:Preposition Categorization**
...
partcoeff.cleaned <- read_excel("C:/Users/daheath/OneDrive - The University of Memphis/Preposition Categorization/0 - Data/For R Final 2.xlsx", sheet = 4)
...
#for reasons unknown, we have to rewrite partcoeff into a new dataframe, or the merge code below wont work - your guess is as good as mine. Only took two hours to figure out. 
...
#mymix_part <- glmer(RResp ~ -1 + PartNum 
...
#mydata_clusters3 <- merge(mydata.cleaned, clusters3.df, by = "PartNum")


```

This chunk writes each group into a seperate data frame for use by the group models below. 
...
#g1.2 <- subset(mydata_clusters3, clusters3.cluster == 1)
#g2.2 <- subset(mydata_clusters3, clusters3.cluster == 2)
#g3.2 <- subset(mydata_clusters3, clusters3.cluster == 3)
```
...
Fixed effects for this model were determined by the IVs: Continuum Step Number (1-9, centered to (-4)-4), Spatial Distinction Familiarity (present in English = Familiar, not present in English = Novel), and Distribution (Overlapping and Non-Overlapping). 
...
Presented here are group models for three groups. These group models are identical to the overall model, except that they only take a subset of the data (the group) as the input. Group data is stored in separate dataframes.

```{r models}

#overall model
mymix <- glmer(RResp ~ centeredStep
...
mymix_g1.1 <- glmer(RResp ~ centeredStep
                    + centeredStep:Distribution
                    + centeredStep:Familiarity
                    + (1+centeredStep|PartNum)
                    + (1+centeredStep | Item),
...
mymix_g2.1 <- glmer(RResp ~ centeredStep 
                    + centeredStep:Distribution 
                    + centeredStep:Familiarity 
                    + (1+centeredStep|PartNum) 
                    + (1+centeredStep|Item), 
...
#mymix_g1.2 <- glmer(RResp ~ centeredStep
 #                   + centeredStep:Distribution
  #                  + centeredStep:Familiarity
   #                 + (1+centeredStep|PartNum)
    #                + (1+centeredStep | Item),
...
#mymix_g2.2 <- glmer(RResp ~ centeredStep 
 #                   + centeredStep:Distribution 
  #                  + centeredStep:Familiarity 
   #                 + (1+centeredStep|PartNum) 
    #                + (1+centeredStep|Item), 
...
#mymix_g3.2 <- glmer(RResp ~ centeredStep
 #                   + centeredStep:Distribution
  #                  + centeredStep:Familiarity
   #                 +  (1+centeredStep|PartNum)
    #                + (1+centeredStep | Item), 
...
#rmat <- round(rbind(summary(mymix)$coefficients[,1], summary(mymix_g1.2)$coefficients[,1], summary(mymix_g2.2)$coefficients[,1], summary(mymix_g3.2)$coefficients[,1]), 4)
#groupnames <- rbind("Overall", "Group 1", "Group 2", "Group 3")
#Logistic_regression_coefficients <- cbind(groupnames, rmat)
#Logistic_regression_coefficients
...
#pmat <- round(rbind(summary(mymix)$coefficients[,4], summary(mymix_g1.2)$coefficients[,4], summary(mymix_g2.2)$coefficients[,4], summary(mymix_g3.2)$coefficients[,4]), 3)
#Logistic_regression_p_values <- cbind(groupnames, pmat)
#Logistic_regression_p_values
```
...
#pred_g1.2 <- predict_response(mymix_g1.2, 
 #                             terms = c("centeredStep", 
  #                                      "Distribution", 
   #                                     "Familiarity"), 
    #                          type="fixed")
...
#pred_g2.2 <- predict_response(mymix_g2.2, 
 #                             terms = c("centeredStep",
  #                                      "Distribution", 
   #                                     "Familiarity"), 
    #                          type="fixed")
...
#pred_g3.2 <- predict_response(mymix_g3.2, 
 #                             terms = c("centeredStep",
  #                                      "Distribution", 
   #                                     "Familiarity"), 
    #                          type="fixed")
...
I think two groups is ok, and does match our predictions relatively well.
...
ggplot(datavis_all, aes(x = centeredStep)) + 
  geom_smooth(aes(y=PredRResp), color = "red") +
                geom_smooth(aes(y=ActRResp)) +
...
ggplot(datavis_g2, aes(x = centeredStep)) + 
  geom_smooth(aes(y=PredRResp), color = "red") +
                geom_smooth(aes(y=ActRResp)) +
                              facet_wrap(Familiarity~Distribution)
#g3
#ggplot(datavis_g3, aes(x = centeredStep)) + 
 # geom_smooth(aes(y=predRResp), color = "red") +
  #              geom_smooth(aes(y=OrigRResp)) +
   #                           facet_wrap(Familiarity~Distribution)
```

Not sure what to say about these yet, but I think they show our fits are pretty good if a little off in a couple places. Also, it is not clear why the actual responses get some kind of error margins but the predicted values don't? 

Open to ideas for making these (and above model plots) more presentable. Obviously I'll want to rename the axes and add titles.
...
And this is our performance chunk
```{r}
#check_model(mymix)
